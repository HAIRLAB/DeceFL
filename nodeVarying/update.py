...
class LocalUpdate(object):
    ...
    def update_weights(self, model, global_round):
        ...
        if self.args.optimizer == 'sgd':
            optimizer = torch.optim.SGD(model.parameters(), lr=self.args.lr, weight_decay=1e-4) #,weight_decay=1e-4 #momentum=0.5
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                if self.args.model == 'logistic':
                    loss = self.criterion(log_probs, labels.float()) + regularization_loss
                else:
                    loss = self.criterion(log_probs, labels.long()) + regularization_loss
    ...
    def inference(self, model):
        ...
        for batch_idx, (images, labels) in enumerate(self.testloader):
            ...
            if self.args.model == 'logistic':
                ...
                correct += (mask.view(-1) == labels).sum().item()  # 计算正确预测的样本个数
            else:
