...
def get_dataset(args):
    ...
    if args.dataset == 'cwru':
        data_train = np.load('../data/CWRU/train.npz')
        data_test = np.load('../data/CWRU/test.npz')
...
def get_varying_w(num, p, method, alg):
    num_sel = int(num / 2)
    if alg == 'defed':
        if method == 'er':
            W_base = erdos_renyi(num_sel, p)
    elif alg == 'fedavg':
        W_base = np.ones(num_sel) / num_sel
    ...
    if num == 8:
        ...
        W1[0:8, 0:8] = W_base
        user1 = [0, 1, 2, 3, 4, 5, 6, 7]
        W2[4:12, 4:12] = W_base
        user2 = [4, 5, 6, 7, 8, 9, 10, 11]
        W3[8:16, 8:16] = W_base
        user3 = [8, 9, 10, 11, 12, 13, 14, 15]
    W_list.append(W1)
    W_list.append(W2)
    W_list.append(W3)
    W_list.append(W2)
    W_list.append(W1)
    sel_list.append(user1)
    sel_list.append(user2)
    sel_list.append(user3)
    sel_list.append(user2)
    sel_list.append(user1)

    return W_list, sel_list
