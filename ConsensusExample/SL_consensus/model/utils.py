...
def get_dataset(args, dataDir):
    """ Returns train and test datasets and a user group which is a dict where
    the keys are the user index and the values are the corresponding data for
    each of those users.
    """
    if args.dataset == 'cwru':
        data_train = np.load(f'{dataDir}/CWRU_c{args.num_classes}/train.npz')
        data_test = np.load(f'{dataDir}/CWRU_c{args.num_classes}/test.npz')
    ...
    if args.iid:
        ...
        user_groups = data_iid_2(train_dataset, args.num_users)
    else:
        # Sample Non-IID user data from Mnist
        if args.unequal:
            # Chose uneuqal splits for every user
            user_groups = data_noniid_example1(train_dataset, args.num_users, args.num_classes)
        else:
            # Chose euqal splits for every user
            user_groups = data_noniid_split_new(train_dataset, args.num_users)
    # print(f'user_groups:{user_groups}')

    return train_dataset, test_dataset, user_groups


def get_data(args, dataDir):
    if args.dataset == 'sl_a':
        # Import Dataset A1/2/3
        # df1 = pd.read_table('f.{data_dir}/A/dataset_A1_RMA.txt')
        # df1 = df1.T
        # info1 = pd.read_table('f.{data_dir}/A/dataset_A1_annotation.txt')

        df2 = pd.read_table(f'{dataDir}/A/dataset_A2_RMA.txt')
        df2 = df2.T
        info2 = pd.read_table(f'{dataDir}/A/dataset_A2_annotation.txt')

        # df3 = pd.read_table('f.{data_dir}/A/dataset_A3_DESeq2.txt')
        # df3 = df3.T
        # info3 = pd.read_table('f.{data_dir}/A/dataset_A3_annotation.txt')
    ...
    if args.dataset == 'sl_b':
        # Import Dataset B
        df = pd.read_table(f'{dataDir}/B/dataset_B_DESeq2_ranktransformed_FG.txt')
        df = df.T
        info = pd.read_table(f'{dataDir}/B/dataset_B_annotation_FG.txt')
    ...
    if args.dataset == 'sl_d':
        # Import Dataset D
        df = pd.read_table(f'{dataDir}/D/dataset_D_DESeq2_ranktransformed_FG.txt')
        df = df.T
        info = pd.read_table(f'{dataDir}/D/dataset_D_annotation_FG.txt')
        info = pd.DataFrame(info)
        info.Condition = info.Condition.map(dict(CASE=1, CONTROL=0))
        info = info.drop(columns=['ID'])
        # print(info)
    ...
    if args.dataset == 'sl_e':
        # Import Dataset E
        df = pd.reafd_table(f'{dataDir}/E/dataset_E_DESeq2_ranktransformed_FG.txt')
        df = df.T
        info = pd.read_table(f'{dataDir}/E/dataset_E_annotation_FG.txt')
        info = pd.DataFrame(info)
        info.Condition = info.Condition.map(dict(CASE=1, CONTROL=0))
        info = info.drop(columns=['ID'])
    ...
    if args.iid:
        ...
        if args.unequal:
            # Chose uneuqal splits for every user
            user_groups = data_noniid_example1(train_dataset, args.num_users)  # raise NotImplementedError()
        else:
            # Chose euqal splits for every user
            user_groups = data_noniid_split_new(train_dataset, args.num_users)
    print(f'user_groups:{user_groups}')
